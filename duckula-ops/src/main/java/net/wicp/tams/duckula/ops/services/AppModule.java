...
public class AppModule {
	public static void bind(ServiceBinder binder) {
		...
		binder.bind(IAjax.class, AjaxImpl.class);
	}
	 ...
	 * how we extend Tapestry using the timing filter. A common use for this kind of
	 * filter is transaction management or security. The @Local annotation selects
	 * the desired service by type, but only from the same module. Without @Local,
	 * there would be an error due to the other service(s) that implement
	...
	public void addTimingFilter(OrderedConfiguration<RequestFilter> configuration, @Local RequestFilter filter) {
		// Each contribution to an ordered configuration has a name, When necessary, you
		// may
		// set constraints to precisely control the invocation order of the contributed
		// filter
		// within the pipeline.
