...
cd $DUCKULA3_HOME
echo "========================access folder======================================"
if [ ! -d "$DUCKULA3_DATA/logs/dump/$1" ]; then
    mkdir -p $DUCKULA3_DATA/logs/dump/$1
fi
...
LOG_ROOT="-DlogRoot=$DUCKULA3_DATA/logs/dump/$1"
JAVA_OPTS="${JAVA_MEM_OPTS} ${JAVA_JMX} ${JAVA_DEFAULT_OPT} ${LOG_ROOT} ${JAVA_DEBUGGER}" 
echo  ${JAVA_OPTS}
echo "============================serverice started============================================="
nohup java  ${JAVA_OPTS}  -jar $DUCKULA3_HOME/duckula-dump.jar  $1 ${startId} ${recordNum} >/dev/null 2>&1 &
echo "========================serverice end======================================"